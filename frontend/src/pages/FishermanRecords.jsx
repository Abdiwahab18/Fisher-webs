...
function FishermanRecords() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ fish_name: '', quantity: '', price: '', location: '', image: '' });
      setShowForm(false);
      loadCatches();
    } catch (err) {
      setError('Unable to save catch.');
    }
  };
      ...
      <main className="flex-1 p-8 overflow-auto">
        ...
        <div className="grid grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-2xl p-6 shadow-sm">
            ...
            <p className="text-green-600 text-xs font-semibold mt-3">This month</p>
          </div>

          <div className="bg-white rounded-2xl p-6 shadow-sm">
            <p className="text-slate-500 text-xs font-semibold uppercase">Total Value</p>
            <p className="text-3xl font-bold text-slate-900 mt-2">
              ${catches.reduce((sum, c) => sum + parseFloat(c.price || 0), 0).toFixed(2)}
            </p>
            <p className="text-green-600 text-xs font-semibold mt-3">Revenue</p>
          </div>

          <div className="bg-white rounded-2xl p-6 shadow-sm">
            <p className="text-slate-500 text-xs font-semibold uppercase">Avg Price/kg</p>
            <p className="text-3xl font-bold text-slate-900 mt-2">
              ${catches.length > 0 ? (catches.reduce((sum, c) => sum + parseFloat(c.price || 0), 0) / catches.length).toFixed(2) : '0.00'}
            </p>
            <p className="text-slate-600 text-xs mt-3">Market rate</p>
          </div>

          <div className="bg-white rounded-2xl p-6 shadow-sm">
            <p className="text-slate-500 text-xs font-semibold uppercase">Active Listings</p>
            <p className="text-3xl font-bold text-slate-900 mt-2">
              {catches.filter(c => c.status !== 'sold').length}
            </p>
        ...
        {showForm && (
          <div className="bg-white rounded-2xl p-6 shadow-sm mb-8">
            <h2 className="text-xl font-bold text-slate-900 mb-4">Log New Catch</h2>
                ...
                {catches.length > 0 ? (
                  catches.map((catch_) => (
                    <tr key={catch_.id} className="border-b border-slate-100 hover:bg-slate-50">
                      ...
                      <td className="py-4 px-4 text-slate-700">{catch_.quantity} kg</td>
                      <td className="py-4 px-4 text-slate-700">${catch_.price}</td>
