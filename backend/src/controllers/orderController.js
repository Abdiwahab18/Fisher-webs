...
import { getFishCatchById, reduceFishCatchWeight, restoreFishCatchWeight } from '../models/catchModel.js';
...
export async function createOrderController(req, res) {
  try {
    ...
    let { delivery_info } = req.body;

    if (!total_price || !items || items.length === 0) {
      return res.status(400).json({ message: 'Total price and items are required' });
    }

    const isDirectPurchaseRole = req.user.role === 'fisherman' || req.user.role === 'admin';

    if (!delivery_info) {
      if (isDirectPurchaseRole) {
        delivery_info = 'Direct Purchase';
      } else {
    ...
    for (const item of items) {
      ...
      if (req.user.role === 'fisherman' && fishCatch.user_id !== req.user.id) {
        return res.status(400).json({ message: 'Fishermen can only purchase their own catches.' });
      }
    ...
    const firstCatch = await getFishCatchById(items[0].fish_id);
    const fishermanId = firstCatch ? firstCatch.user_id : null;
    ...
    const notifMsg = isDirectPurchaseRole
      ? `Your purchase (Order #${order.id}) is pending payment. Please complete your checkout.`
    ...
    if (fishermanId && fishermanId !== req.user.id) {
      await createNotification({
        user_id: fishermanId,
        message: `Order #${order.id} for your catch has been placed and completed by ${req.user.name || 'a buyer'}.`,
        is_read: false
      });
    }
    ...
    if (io) {
      ...
      try {
        if (items && items.length > 0) {
          qty = items[0].weight;
          const res = await pool.query('SELECT fs.name AS fish_name FROM fish_catches fc JOIN fish_species fs ON fc.fish_species_id = fs.id WHERE fc.id = $1', [items[0].fish_id]);
...
export async function updateOrderStatusController(req, res) {
  try {
    ...
    if (['out_for_delivery', 'delivered', 'completed'].includes(status) && !previousOrder.driver_id) {
      return res.status(400).json({ message: 'Please assign a driver to this order before changing its status to out for delivery, delivered, or completed.' });
    }
...
export async function getOrderItemsController(req, res) {
  try {
...
export async function acceptDelivery(req, res) {
  try {
    // Drivers cannot self-accept deliveries anymore. Fisherman must manually assign them.
    if (req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Forbidden: Drivers cannot self-assign orders. The fisherman must assign drivers manually.' });
    }
        ...
        if (currentOrder.status === 'pending') {
          ...
          if (paymentCheck.rows.length === 0) {
            await pool.query(
              ...
              [orderId, currentOrder.user_id, currentOrder.total_price, 'autopopilot', `AP_${Date.now()}`, 'Autopilot', 'verified']
            );
          }
